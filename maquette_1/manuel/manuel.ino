...
int potPin             =   2;     // Broche signal du potentiometre
...
void loop() {
  ...
  // Lecture valeur vitesse sur le potentiometre et envoi au moteur
  vitesse = map(analogRead(potPin), 0, 681, 0, 255);   // Lecture vitesse entre 0 et 1023 et recalibrage entre 0 et 255
  ...
  // Test des boutons
  if ((vertState == HIGH) and (digitalRead(ilsDroitPin) == 0)) {
    deplacementDroite();
  }
  else if ((rougeState == HIGH) and (digitalRead(ilsGauchePin) == 0)) {
    deplacementGauche();
  }
  else {
    arretMoteur();
